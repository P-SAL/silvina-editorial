"""
main_legacy.py
Legacy main entry point for Silvina Editorial Assistant v0.9
Orchestrates the complete document analysis workflow using legacy layered architecture.
"""

# ruff: noqa: E402 — frozen snapshot of the pre-Slice-14 main.py; import order
# preserved verbatim (sys.path setup must run before the layered imports below).
...
class SilvinaEditorialAssistant:
    """Main orchestrator for the Silvina Editorial Assistant (Legacy)."""
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        print("🔧 Inicializando Silvina Editorial Assistant v0.9 (Legacy)...")
        ...
        try:
            ...
            self.article_classifier = ArticleClassifier(
                model_name=self.config.ollama_model, base_url=self.config.ollama_base_url
            )
    ...
    def analyze_document(self, document_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print("      ✓ Documento leído correctamente")

            # Step 2: Extract content
            print("\n[2/7] 🔍 Extrayendo contenido estructurado...")
            document_content = self.content_extractor.extract_content(paragraphs, document_path)
            ...
            # Count only author-year citations
            author_year_count = len(
                [c for c in citations if c.citation_type.value == "author_year"]
            )
            ...
            if len(citation_tuples) == 0:
                print("      ℹ️  No se detectaron citas en texto para validar formato APA")
                apa_violations = []
                apa_report = ""
            else:
                ...
                if apa_violations:
                    ...
                    print(
                        f"      ✅ Formato APA 7 correcto ({len(citation_tuples)} citas validadas)"
                    )

            # 3.6: Check spelling/grammar
            print("\n      🔍 Validando gramática ...")
            try:
            ...
            quality_result = self.quality_analyzer.analyze_quality(document_content, classification)
            ...
            imryd_classified = (
                classification.article_type == ArticleType.CIENTIFICO
                and "IMRyD" in (classification.reasoning or "")
            )
            effective_type = (
                classification.article_type
                if imryd_classified
                else ArticleType.DIVULGACION
            ...
            citation_analysis = self.citation_matcher.match_citations_to_references(section_type)
    ...
    def _generate_recommendations(
        ...
        if (
            quality_result.overall_score < 5.0
            or gramatica_score < 5.0
            or not structure_result.is_valid
            or match_rate < 50
        ):
        ...
        if (
            ...
            or gramatica_score < 7.0
            or match_rate < 90
            or len(apa_violations) > 0
        ):
            has_warnings = True
...
def main():
    ...
    try:
        ...
        if gramatica_errors:
            print("        Detalles:")
            for err in gramatica_errors[:5]:
                context = (
                    err["context"][:50] + "..." if len(err["context"]) > 50 else err["context"]
                )
                print(f"          • {err['message']}")
        ...
        total_citations = len(
            [c for c in results["citations"] if c.citation_type.value == "author_year"]
        )
        print(f"\n  📚 CITAS: {total_citations} detectadas")

        footnote_count = len(
            [c for c in results["citations"] if c.citation_type.value == "footnote"]
        )
        if footnote_count > 0:
