...
**S7e — Paragraph Index Assignment**: GIVEN a document with citations in multiple paragraphs | WHEN `extract_citations(docx_path)` is called | THEN each citation has its `location` set to its correct 0-based paragraph index.
**S7f — Raw Text Fallback**: GIVEN a raw text string or `full_text` fallback | WHEN citations are extracted using `_extract_citations` | THEN each citation has its `location` set to 0.
