...
- [x] **T-01** Confirm `src/domain/citation/__init__.py` and `src/domain/tests/citation/__init__.py`
...
- [x] **T-02** RED: write tests for `_normalize_author` non-author sentinel cases — one test per
...
- [x] **T-03** GREEN: implement `CitationMatcher` class skeleton with `_NON_AUTHOR_PATTERNS` class
...
- [x] **T-04** RED: write tests for `_normalize_author` real-author normalization edge cases —
...
- [x] **T-05** GREEN: complete `_normalize_author` body (year-parenthetical strip, et-al strip,
  `" y "` strip, initials strip, punctuation strip, lowercase first word) to pass T-04.
  - Satisfies: Requirement "Author Normalization Preserved Verbatim"
  - Sequential: depends on T-04, builds on T-03

- [x] **T-06** RED: write tests for `_citable()` helper — given a mixed list of footnote
...
- [x] **T-07** GREEN: implement `_citable()` private helper (`citation_type != CitationType.FOOTNOTE
  and citation.author` filter) to pass T-06.
  - Satisfies: same as T-06
  - Sequential: depends on T-06

- [x] **T-08** RED: write tests for `_build_citation_keys` and `_build_reference_keys` — assert
...
- [x] **T-09** GREEN: implement `_build_citation_keys` and `_build_reference_keys` to pass T-08.
  - Sequential: depends on T-08

- [x] **T-10** RED: write tests for `_is_orphaned_citation` — citation whose normalized author is
...
- [x] **T-11** GREEN: implement `_is_orphaned_citation` to pass T-10.
  - Sequential: depends on T-10

- [x] **T-12** RED: write tests for `find_orphaned_citations` — empty citations/references yields
...
- [x] **T-13** GREEN: implement `find_orphaned_citations` (delegates to `_citable`,
  `_build_reference_keys`, `_is_orphaned_citation`) to pass T-12.
  - Sequential: depends on T-12

- [x] **T-14** RED: write tests for `find_orphaned_references` — all references cited yields empty
...
- [x] **T-15** GREEN: implement `find_orphaned_references` (delegates to `_build_citation_keys`)
  to pass T-14.
  - Sequential: depends on T-14, T-09

- [x] **T-16** RED: write tests for `match_citations_to_references` — empty inputs yield
...
- [x] **T-17** GREEN: implement `match_citations_to_references` (delegates to
...
- [x] **T-18** RED: write a statelessness regression test — instantiate one `CitationMatcher`,
...
- [x] **T-19** GREEN: confirm T-18 passes with no code change (statelessness is structural —
...
- [x] **T-20** RED: write test asserting `MatchCitationsUseCase.execute(citations, references,
...
- [x] **T-21** GREEN: implement `MatchCitationsUseCase.__init__(self, matcher: CitationMatcher)`
...
- [x] **T-22** RED: write tests mirroring `test_validate_apa_wiring.py` — `create_use_case()`
...
- [x] **T-23** GREEN: implement `MatchCitationsUseCaseWiring` with `create_use_case()` public
...
- [x] **T-24** Run `python -m pytest src/` and confirm 0 regressions: baseline 209 tests + all new
...
- [x] **T-25** Quick manual review pass against `clean-architecture/SKILL.md`: confirm no
