...
def analyze_feedback(records: list) -> dict:
    ...
    return {
        ...
        "classification": Counter(
            r.get("classification_correct", "Sin respuesta") for r in records
        ),
...
def identify_issues(data: dict) -> list:
    ...
    if too_high > 0 or too_low > 0:
        ...
        issues.append(
            {
                "count": too_high + too_low,
                "pct": round((too_high + too_low) / n * 100),
...
def generate_summary(data: dict, issues: list) -> str:
    ...
    if not issues:
        ...
        for i, issue in enumerate(issues, 1):
            priority = (
                "CRITICO" if issue["pct"] >= 50 else "MODERADO" if issue["pct"] >= 25 else "MENOR"
            )
...
def generate_dev_prompt(data: dict, issues: list) -> str:
    ...
    for i, issue in enumerate(issues, 1):
        ...
        lines.append(
            f"### Fix {i} [{priority}] - {issue['component']} ({issue['count']} cases, {issue['pct']}%)"
        )
...
def main():
    ...
    parser.add_argument("--output", type=str, default=None, help="Output filename for summary")
    ...
    summary_file = (
        Path(args.output)
        if args.output
        else Path(f"feedback_summary_{datetime.now().strftime('%Y%m%d')}.md")
    )
